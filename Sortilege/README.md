...
The window opens in a dark theme by default, matched to UEFN's own dark editor so it does not blind you mid-session. Prefer the plain system (light) look? Set `"DARK_MODE": False` in the CONFIG section near the top of `sortilege.py`. This is purely cosmetic — every control works identically either way, and on builds whose tkinter cannot apply some part of the styling, that part simply keeps its default colors instead of causing any problem.
